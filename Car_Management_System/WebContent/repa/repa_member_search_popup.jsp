...
<script type = "text/javascript" src="js/repa.js?ver=3"></script>
<title>사원 검색</title>
</head>
<body>
...
<div class = "popup_body">
	<c:choose>
		<c:when test = "${isLike eq 'no'}">
			<div class = "popup_box">
				<form name = "frm" method = "post" action = "repa.do?command=repa_member_search&popup=yes">
					사원 이름 &nbsp;
		...
		<c:when test = "${isLike eq 'yes'}">
			...
			<div class = "popup_box">
				...
				<table class = "table table-condensed table-bordered">					
					...
					<tr>
						<td>${mem.mem_id}</td>
						<td><a onclick = "repaMemSelect('${mem.mem_id}','${mem.mem_name}')">${mem.mem_name}</a></td>		
						<td>${mem.dept_name}</td>
						<td>${mem.mem_posi}</td>
					</tr>					
