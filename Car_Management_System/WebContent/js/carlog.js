...
function carNoCheck() {
   ...
   document.frm.car_model.value = ""; // 재클릭 시 기존 검색  삭제
   document.frm.car_reg_no.value = ""; // 재클릭 시 기존 검색  삭제
}
  function carlogDateCheck() {
	...
	/*	else if(car_reg_no =="" || mem_name=""){
		...
		/*}
			 ...
			 var url = "carlog.do?command=carlog_view_form";  
				document.frm.action=url; 	
				//document.frm.submit();
			return true;
		}
...
function memSearchByName() {
	...
	window.open(url, "_blank_1",
	   ...
	   document.frm.mem_id.value = ""; // 재클릭 시 기존 검색  삭제
